...
import { monthKey, canAwardManagerRule, acknowledgePolicy, awardCertDetail, awardCertsCurrent, certAlertLevelFromType, confirmIncidentReceipt, newHirePolicySignDue, policyAdminUpdateReminderActive, policyDueDate, recordTimeOff, rulePoints, submitIncidentReport, submitOnboarding, timeOffSummary, vacationReminderText } from "../domain/crew";
...
import { privateDocSignedUrl, uploadCertMedia, uploadPrivateDoc } from "../data/repo";

type WorkTab = "profile" | "onboarding" | "timeoff" | "incidents" | "certs";

type UploadStatus = "" | "uploading" | "done" | "failed";

const UPLOAD_STATUS_TEXT: Record<Exclude<UploadStatus, "">, string> = {
  uploading: "Uploading...",
  done: "Uploaded.",
  failed: "Upload failed - nothing was saved. Try again.",
};

// Stored files are keyed as <scope>/<user id>/<uuid>-<original name>, so show
// people the name they picked rather than the key we route it by.
...
function ImageFilePicker({ accept, value, status, onChange }: { accept?: string; value?: string; status?: UploadStatus; onChange: (file: File | null) => void }) {
  ...
  return <div className="file-picker">
    ...
    {previewUrl ? <img src={previewUrl} alt="Selected file preview" className="file-preview-thumb" /> : value ? <small className="muted">Selected: {fileLabel(value)}</small> : null}
    {status ? <small className={status === "failed" ? "bad" : "muted"}>{UPLOAD_STATUS_TEXT[status]}</small> : null}
  </div>;
}

// Private-doc buckets are admin-read only, so the file is fetched through a
...
function PrivateDocLink({ storageKey, label }: { storageKey: string; label: string }) {
  ...
  return <span className="doc-link">
    <button type="button" onClick={open} disabled={status === "opening"}>{status === "opening" ? "Opening..." : label}</button>
    {status === "failed" ? <small className="bad">Could not open this file.</small> : null}
  </span>;
}
...
type OnboardingDocField = "directDepositFileName" | "driversLicenseFrontFileName" | "driversLicenseBackFileName";
...
function Onboarding({ state, user, setState }: { state: CrewState; user: Profile; setState: React.Dispatch<React.SetStateAction<CrewState>> }) {
  ...
  const [docStatus, setDocStatus] = useState<Partial<Record<OnboardingDocField, UploadStatus>>>({});
  const attachDoc = async (field: OnboardingDocField, file: File | null) => {
    const mark = (status: UploadStatus) => setDocStatus((current) => ({ ...current, [field]: status }));
    if (!file) { set({ [field]: "" } as Partial<OnboardingDraft>); mark(""); return; }
    if (!REMOTE_MODE) { set({ [field]: file.name } as Partial<OnboardingDraft>); mark("done"); return; }
    mark("uploading");
    try {
      set({ [field]: await uploadPrivateDoc("onboarding", user.id, file) } as Partial<OnboardingDraft>);
      mark("done");
    } catch {
      set({ [field]: "" } as Partial<OnboardingDraft>);
      mark("failed");
    }
  };
...
function Incidents({ state, user, setState }: { state: CrewState; user: Profile; setState: React.Dispatch<React.SetStateAction<CrewState>> }) {
  ...
  const [photoStatus, setPhotoStatus] = useState<UploadStatus>("");
  const attachPhoto = async (file: File | null) => {
    if (!file) { set({ photoFileNames: [] }); setPhotoStatus(""); return; }
    if (!REMOTE_MODE) { set({ photoFileNames: [file.name] }); setPhotoStatus("done"); return; }
    setPhotoStatus("uploading");
    try {
      set({ photoFileNames: [await uploadPrivateDoc("incidents", user.id, file)] });
      setPhotoStatus("done");
    } catch {
      set({ photoFileNames: [] });
...
function Compliance({ state, user, setState }: { state: CrewState; user: Profile; setState: React.Dispatch<React.SetStateAction<CrewState>> }) {
  ...
  return <div className="compliance-page">
    ...
    <div className="library">{visibleUsers.map((item) => <ComplianceUserCard key={item.id} state={state} user={item} viewer={user} setState={setState} />)}</div>
  </div>;
}
...
// `user` is the person the card is about; `viewer` is whoever is signed in.
function ComplianceUserCard({ state, user, viewer, setState }: { state: CrewState; user: Profile; viewer: Profile; setState: React.Dispatch<React.SetStateAction<CrewState>> }) {
  ...
  const [certStatus, setCertStatus] = useState<Record<string, UploadStatus>>({});
  const attachCertMedia = async (certId: string, file: File | null) => {
    const mark = (status: UploadStatus) => setCertStatus((current) => ({ ...current, [certId]: status }));
    if (!file) { updateCert(certId, { certificatePhotoKey: undefined }); mark(""); return; }
    if (!REMOTE_MODE) { updateCert(certId, { certificatePhotoKey: file.name }); mark("done"); return; }
    mark("uploading");
    try {
