  ...
  <section className="panel card"><h3>{crewScale ? "Crew scale" : "Office scale"}</h3><div className="seg review-scale" role="img" aria-label={crewScale ? "Rating scale: Below, Meets, Exceeds" : "Rating scale: 1 to 5"}>{(crewScale ? ["Below", "Meets", "Exceeds"] : ["1", "2", "3", "4", "5"]).map((label) => <span key={label}>{label}</span>)}</div><p className="muted">Crew reviews use Below, Meets, and Exceeds. Office roles may use the optional 1-5 scale.</p></section><section className="panel card wide"><div className="section-head"><h3>KPIs</h3><span className="pill">{user.orgRole}</span></div>{state.kpis.filter((kpi) => kpi.role === user.orgRole).map((kpi) => {
    ...
    return <div className="line" key={kpi.id}><b>{kpi.name}</b><span>{kpi.target || "Target TBD"}</span>
      {hit ? <span className="pill good">Hit</span>
        : canAwardManagerRule(user) ? <button onClick={() => { setState((next) => awardKpiHit(next, user.id, kpi.id, quarterKey())); showToast(`KPI marked hit. +${rulePoints(state, "earn-kpi", 5)} points.`); }}>Mark hit +{rulePoints(state, "earn-kpi", 5)}</button>
...
function Bonus({ state, user, setState }: { state: CrewState; user: Profile; setState: React.Dispatch<React.SetStateAction<CrewState>> }) {
  ...
  // Both of these used to be pinned to user.id, so an admin could only ever read
  // and write their OWN wage record - there was no way to enter anyone else's, and
  // the bonus program computes its dollar figures from gross annual wages. The
  // domain function and the RLS policy both already took a separate subject id.
  const compSubjects = state.users.filter((item) => item.status !== "Inactive");
  const [compUserId, setCompUserId] = useState(user.id);
  const compSubject = compSubjects.find((item) => item.id === compUserId) ?? user;
  const comp = (state.compensation ?? []).find((item) => item.userId === compSubject.id);
  ...
  const updateComp = (patch: Parameters<typeof setCompensation>[3]) => setState((next) => setCompensation(next, next.currentUserId, compSubject.id, patch));
