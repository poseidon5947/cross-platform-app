...
import { approveRedemption, awardCertsCurrent, awardKpiHit, bonusTrajectory, canApproveRedemptions, canRunReviews, canSeeBonusDollars, certAlertLevel, certAlertLevelFromType, completeReview, completeRitual, confirmCustomerReview, estimatedBonusDollars, giveRecognition, hasRolePermission, impliedRewardValue, isRedemptionWindowOpen, leaderboard, nextRedemptionWindow, quarterlyLeaderboard, requestRedemption, rulePoints, submitFeedback, walletBalance } from "./domain/crew";
...
function Home({ state, user, setTab }: { state: CrewState; user: Profile; setTab: (tab: "wallet" | "certs" | "reviews" | "bonus") => void }) {
  ...
  return <div className="grid two"><section className="panel card hero-panel"><h3>{walletBalance(state.pointsEvents, user.id)} points</h3><p>Shared wallet from Warehouse Wizard, SOP+, and Crew+.</p><div className="hero-actions"><button className="primary" onClick={() => setTab("wallet")}>Open wallet</button><button className="primary" onClick={() => setTab("certs")}>Compliance</button></div></section><section className="panel card"><div className="section-head"><h3>Bonus trajectory</h3><span className={`status ${trajectory}`}>{trajectory.toUpperCase()}</span></div><p className="muted">Trajectory is visible to everyone. Dollars stay admin/CFO-only.</p><div className="score-grid"><div className="score-tile"><strong>{state.reviews.filter((review) => review.userId === user.id && review.status === "completed").length}</strong><span>completed reviews</span></div><div className="score-tile"><strong>{state.certifications.filter((cert) => cert.userId === user.id).length}</strong><span>cert records</span></div></div><button onClick={() => setTab("bonus")}>Open scorecard</button></section><section className="panel card"><h3>Leaderboard</h3>{board.map((row, index) => <div className="lb" key={row.user.id}><b>{index + 1}. {row.user.name}</b><span className="pill">{row.balance} pts</span></div>)}</section><section className="panel card"><h3>Today's nudge</h3><p>{state.values[0].dailyRitual}</p><div className="facts"><span>{state.values[0].name}</span><span>Daily ritual</span></div><button onClick={() => setTab("reviews")}>Review cadence</button></section></div>;
}
...
function Rituals({ state, user, setState }: { state: CrewState; user: Profile; setState: React.Dispatch<React.SetStateAction<CrewState>> }) {
  ...
  return <div className="library">{state.values.map((value) => <section className="panel card" key={value.id}><div className="section-head"><div><h3>{value.name}</h3><p>{value.wording}</p></div><span className="pill">Value</span></div><div className="ritual"><b>Daily</b><span>{value.dailyRitual}</span><button onClick={() => setState((next) => completeRitual(next, user.id, value.id, "daily", "2026-W31-Mon"))}>Complete +{pointsFor("daily")}</button></div><div className="ritual"><b>Weekly</b><span>{value.weeklyRitual}</span><button onClick={() => setState((next) => completeRitual(next, user.id, value.id, "weekly", "2026-W31"))}>Complete +{pointsFor("weekly")}</button></div><div className="ritual"><b>Monthly</b><span>{value.monthlyRitual}</span><button onClick={() => setState((next) => completeRitual(next, user.id, value.id, "monthly", "2026-07"))}>Complete +{pointsFor("monthly")}</button></div></section>)}</div>;
}
...
function Reviews({ state, user, setState }: { state: CrewState; user: Profile; setState: React.Dispatch<React.SetStateAction<CrewState>> }) {
  ...
  return <div className="grid two"><section className="panel card wide"><div className="section-head"><h3>Review cadence</h3><span className="pill">{visible.length} visible</span></div>{visible.map((review) => <div className="review-card" key={review.id}><div><b>{nameOf(state, review.userId)} - {review.type}</b><small>{review.scheduledFor} - {review.status}</small></div>{review.status !== "completed" && canRunReviews(state, user) && <button onClick={() => setState((next) => completeReview(next, review.id, { responsibilities: "meets", values: "meets", kpis: "meets" }))}>Complete +{rulePoints(state, "earn-review", 5)}</button>}</div>)}</section><section className="panel card"><h3>Scale</h3><div className="seg review-scale"><button>Below</button><button className="on">Meets</button><button>Exceeds</button></div><p className="muted">Crew reviews use the 3-tier scale. Office roles can keep an optional 1-5 view later.</p></section><section className="panel card wide"><div className="section-head"><h3>KPIs</h3><span className="pill">{user.orgRole}</span></div>{state.kpis.filter((kpi) => kpi.role === user.orgRole).map((kpi) => <div className="line" key={kpi.id}><b>{kpi.name}</b><span>{kpi.target || "Target TBD"}</span><button onClick={() => setState((next) => awardKpiHit(next, user.id, kpi.id, "2026-Q3"))}>Mark hit +{rulePoints(state, "earn-kpi", 5)}</button></div>)}</section><section className="panel card wide"><h3>Between-review notes</h3>{state.reviewNotes.length ? state.reviewNotes.map((note) => <div className="line" key={note.id}><b>{nameOf(state, note.userId)}</b><span>{note.note}</span><small>{note.ts.slice(0, 10)}</small></div>) : <p className="empty-state">No lightweight notes yet.</p>}</section></div>;
}
...
function Bonus({ state, user, setState }: { state: CrewState; user: Profile; setState: React.Dispatch<React.SetStateAction<CrewState>> }) {
  ...
  const trajectory = bonusTrajectory(state.reviews.filter((review) => review.userId === user.id && review.status === "completed").flatMap((review) => Object.values(review.ratings).filter(Boolean) as ReviewRating[]));
  return <div className="grid two"><section className="panel card hero-panel"><h3>{trajectory.toUpperCase()}</h3><p>On-track indicator based on review ratings. December payout timing stays.</p><div className="facts"><span>{state.bonusPeriods[0]?.year ?? 2026} period</span><span>{state.bonusPeriods[0]?.status ?? "draft"}</span></div></section><section className="panel card"><h3>Config</h3><div className="field-stack"><label>Profit pool %<input value={state.bonusConfig.profitSharePercent} onChange={(event) => setState((next) => ({ ...next, bonusConfig: { ...next.bonusConfig, profitSharePercent: Number(event.target.value) } }))} /></label><p className="muted">{state.bonusConfig.floorsCaps}</p></div></section><section className="panel card wide"><div className="section-head"><h3>Dollar privacy</h3><span className="pill">{dollarsVisible ? "Admin/CFO" : "Private"}</span></div>{dollarsVisible ? <Metric label="Estimated share" value={`$${estimatedBonusDollars(state, user).toLocaleString("en-CA")}`} /> : <p className="empty-state">Your trajectory is visible, but bonus dollar amounts are admin/CFO-only.</p>}</section></div>;
}
