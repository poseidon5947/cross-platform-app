...
export function ToastProvider({ children }: { children: React.ReactNode }) {
  ...
  const showToast = useCallback((message: string, tone: ToastTone = "good") => { const id = Date.now() + Math.random(); setItems((current) => [...current, { id, message, tone }]); window.setTimeout(() => dismissToast(id), 3000); }, [dismissToast]);
